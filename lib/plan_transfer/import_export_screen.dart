...
class _ImportExportScreenState extends State<ImportExportScreen> {
          ...
          children: [
            ...
            _SectionTitle(
              ...
              subtitle: _usesExternalIosImport
                  ? '从文件、聊天、浏览器或网盘等位置用 ProjectTabi 打开 .sjhplan。'
            ...
            _ActionTile(
              ...
              title: _importing
                  ...
                  : '导入 ProjectTabi 文件',
              subtitle: _usesExternalIosImport
                  ? '在文件、聊天、浏览器下载页或网盘中选择 .sjhplan，然后分享或用 ProjectTabi 打开。'
                  : '兼容 MiriaGo v2 数据包和旧版 v1 JSON 计划包。',
      ...
      final file = await file_selector.openFile(
        acceptedTypeGroups: const [
          file_selector.XTypeGroup(
            label: 'ProjectTabi plan package',
  ...
  Future<void> _showExternalIosImportHelp() async {
    await showInfoActionDialog(
      ...
      message:
          '请在文件、聊天、浏览器下载页、网盘或其他保存位置找到 .sjhplan 文件，然后点开文件，或使用分享/更多菜单选择 ProjectTabi。\n\n'
          'ProjectTabi 收到文件后会自动进入导入预览页面。若列表里没有 ProjectTabi，可以先把文件保存到“文件”App，再长按文件选择分享或打开方式。',
    );
  }
  ...
  Future<void> _exportV2() async {
    await _runExport((generation) async {
      ...
      final result = await deliverPlanExport(
        ...
        shareText: 'ProjectTabi数据包：${widget.plan.name}',
        extension: seichiPlanFileExtension,
  ...
  Future<void> _exportMyMapsCsv() async {
    await _runExport((generation) async {
      ...
      final result = await deliverPlanExport(
        ...
        mimeType: export.mimeType,
        shareSubject: widget.plan.name,
        shareText: 'ProjectTabi My Maps CSV：${widget.plan.name}',
        extension: myMapsCsvExtension,
        destination: destination,
      );
      return _PlanExportRunResult(
      ...
      child: Column(
        ...
        children: [
          ...
          FilledButton.icon(
            ...
            label: Text(exporting ? '导出中...' : '导出 ProjectTabi 数据包'),
          ),
        ],
      ),
    );
