...
Future<String?> ensureReferenceThumbnailCached(
  ...
  if (existingPath != null &&
      ...
      (_cachedPathMatchesUrl(existingPath, thumbnailUrl) ||
          ...
          _isSyncedThumbnailPath(existingPath))) {
...
// A synchronized thumbnail already belongs to this point's reference URL.
// Its content hash is not the URL hash used by the download cache.
bool _isSyncedThumbnailPath(String path) {
  final normalized = path.replaceAll(r'\', '/');
  return RegExp(
    r'(^|/)(webdav_sync_assets|assets/webdav_sync)/[a-f0-9]{64}\.[a-z0-9]+$',
  ).hasMatch(normalized);
}
