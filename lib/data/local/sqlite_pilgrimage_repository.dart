...
class SqlitePilgrimageRepository implements PilgrimageRepository, SyncRepository {
  ...
  @override
  Future<void> replaceSyncSnapshot(SamplePilgrimageRepositorySnapshot state) async {
    if (state.plans.isEmpty) throw StateError('至少需要保留一个计划。');
    await _database.transaction(() async {
      await _database.delete(_database.visitRecords).go();
      await _database.delete(_database.points).go();
      await _database.delete(_database.planGroups).go();
      await _database.delete(_database.works).go();
      await _database.delete(_database.plans).go();
      for (final plan in state.plans) {
        await _insertPlan(plan, active: plan.id == state.activePlanId);
      }
      for (final record in state.visitRecords) {
        await _database
