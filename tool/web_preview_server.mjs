...
const host = process.env.PROJECT_TABI_PREVIEW_HOST ?? '127.0.0.1';
const port = Number.parseInt(process.env.PROJECT_TABI_PREVIEW_PORT ?? '8791', 10);
...
async function fetchAnitabiStatic(fileName, version, upstreamValue) {
  ...
  const response = await fetch(`${baseUrl}/${fileName}${query}`, {
    headers: {'user-agent': 'ProjectTabi local web preview'},
  });
...
server.listen(port, host, () => {
  console.log(`ProjectTabi preview: http://${host}:${port}/`);
